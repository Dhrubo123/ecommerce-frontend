...
export default function EcommerceOrderForm() {
  ...
  const submit = async (event) => {
    ...
    try {
      await createEcommerceOrder({ warehouseId: Number(form.warehouseId), customer: form.customer, paymentMethod: form.paymentMethod === 'cash' ? 'cod' : form.paymentMethod, shippingCost: Number(form.shippingCost), discount: Number(form.discount), note: form.note, items: form.items.map((item) => ({ productId: Number(item.productId), quantity: Number(item.quantity) })) })
      navigate('/orders')
  ...
  return <AdminLayout title="Create Order"><div className="brand-page"><div className="brand-heading"><div><p>ORDER MANAGEMENT</p><h2>Create Order</h2><span>Create an online order with delivery and payment information.</span></div></div><form className="brand-form" onSubmit={submit}><section><h3>Order information</h3>{error && <div className="brand-error">{error}</div>}
    ...
    <div className="brand-form-grid"><label>Payment Method <select value={form.paymentMethod} onChange={(event) => setForm((current) => ({ ...current, paymentMethod: event.target.value }))}><option value="cod">Cash on Delivery</option><option value="card">Card</option><option value="mobile_banking">Mobile Banking</option><option value="bank_transfer">Bank Transfer</option></select></label><label>Shipping Cost <input type="number" min="0" value={form.shippingCost} onChange={(event) => setForm((current) => ({ ...current, shippingCost: event.target.value }))} /></label></div>
