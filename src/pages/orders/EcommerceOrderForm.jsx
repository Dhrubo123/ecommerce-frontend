...
export default function EcommerceOrderForm() {
  ...
  return <AdminLayout title="Create Ecommerce Order"><div className="brand-page"><div className="brand-heading"><div><p>ORDER MANAGEMENT</p><h2>Create Ecommerce Order</h2><span>Create an online order with delivery and payment information.</span></div></div><form className="brand-form" onSubmit={submit}><section><h3>Order information</h3>{error && <div className="brand-error">{error}</div>}
    ...
    <div className="brand-form-actions"><button type="button" onClick={() => navigate('/ecommerce-orders')}>Cancel</button><button className="brand-primary" disabled={saving}>{saving ? 'Creating…' : 'Create Order'}</button></div>
