  ...
  return <AdminLayout title="Orders"><div className="brand-page">
    ...
    <section className="brand-card"><div className="brand-toolbar"><label><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search order, customer, or phone" /></label></div>{error && <div className="brand-error">{error}</div>}
      <div className="brand-table"><table><thead><tr><th>Order</th><th>Customer</th><th>Phone</th><th>Warehouse</th><th>Payment</th><th>Total</th><th>Status</th><th>Created</th></tr></thead><tbody>
        {loading ? <tr><td colSpan="8">Loading orders…</td></tr> : orders.length === 0 ? <tr><td colSpan="8">No orders found.</td></tr> : orders.map((order, index) => <tr key={order.id || index}><td><strong>#{order.orderNumber || order.id || index + 1}</strong></td><td>{order.customer?.name || order.customerName || '—'}</td><td>{order.customer?.phone || order.phone || '—'}</td><td>{order.warehouse?.name || order.warehouseName || '—'}</td><td>{order.paymentMethod || '—'}</td><td>{money(order.totalAmount ?? order.total)}</td><td><span className="brand-status active">{order.status || 'Pending'}</span></td><td>{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : '—'}</td></tr>)}
      </tbody></table></div>
    </section>
  </div></AdminLayout>
}
