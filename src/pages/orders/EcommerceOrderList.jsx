...
export default function EcommerceOrderList() {
  ...
  const visibleOrders = useMemo(() => orders.filter((order) => {
    ...
    const matchesDate = !orderDate || String(order.createdAt ?? order.orderDate ?? '').slice(0, 10) === orderDate
    return matchesSearch && matchesStatus && matchesWarehouse && matchesCustomer && matchesDate
  }), [orders, search, status, warehouseId, customerId, orderDate])
  ...
  return <AdminLayout title="Orders"><div className="brand-page">
    ...
    <section className="brand-card"><div className="order-filters"><label><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search order, customer, or phone" /></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{statuses.map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</select></label><label>Warehouse<select value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)}><option value="">All warehouses</option>{options.warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></label><label>Customer<select value={customerId} onChange={(event) => setCustomerId(event.target.value)}><option value="">All customers</option>{options.customers.map((customer) => <option key={customer.id} value={customer.id}>{`${customer.firstName ?? ''} ${customer.lastName ?? ''}`.trim() || customer.name || `Customer #${customer.id}`}</option>)}</select></label><label>Order date<input type="date" value={orderDate} onChange={(event) => setOrderDate(event.target.value)} /></label></div>{error && <div className="brand-error">{error}</div>}
      <div className="brand-table"><table><thead><tr><th>Order</th><th>Customer</th><th>Phone</th><th>Warehouse</th><th>Payment</th><th>Total</th><th>Status</th><th>Created</th><th>Action</th></tr></thead><tbody>
        {loading ? <tr><td colSpan="9">Loading orders…</td></tr> : visibleOrders.length === 0 ? <tr><td colSpan="9">No orders match these filters.</td></tr> : visibleOrders.map((order, index) => <tr key={order.id || index}><td><strong>#{order.orderNumber || order.id || index + 1}</strong></td><td>{order.customer?.name || order.customerName || '—'}</td><td>{order.customer?.phone || order.phone || '—'}</td><td>{order.warehouse?.name || order.warehouseName || '—'}</td><td>{order.paymentMethod || '—'}</td><td>{money(order.totalAmount ?? order.total)}</td><td><span className={`brand-status ${String(order.status || '').toLowerCase() === 'cancelled' ? 'inactive' : 'active'}`}>{order.status || 'Pending'}</span></td><td>{order.createdAt ? new Date(order.createdAt).toLocaleDateString() : '—'}</td><td><div className="brand-actions"><Link to={`/orders/${order.id}`} title="View order"><Eye size={15} /></Link><Link to={`/orders/${order.id}/invoice`} title="View invoice" aria-label="View invoice"><FileText size={15} /></Link></div></td></tr>)}
      </tbody></table></div>
    </section>
  </div></AdminLayout>
}
