...
import { createPosDraft, createPosSale, deletePosDraft, updatePosDraft } from '../../services/posSaleService'
...
// Product relations can return a join-row ID as well as the actual size/color
// ID. The POS API requires the latter, so prefer sizeId/colorId first.
const firstOptionId = (product, keys) => { for (const key of keys) { const values = product[key]; const first = Array.isArray(values) ? values[0] : values; const id = Number(first?.sizeId ?? first?.colorId ?? first?.size_id ?? first?.color_id ?? first?.id ?? first); if (Number.isFinite(id) && id > 0) return id } return null }
const saleDateTime = () => {
  const date = new Date()
...
export default function PosSaleForm() {
  ...
  useEffect(() => {
    ...
    setCart(draftItems.map((item) => ({ productId: Number(item.productId ?? item.product_id ?? item.product?.id), name: item.product?.name || item.productName || `Product #${item.productId ?? item.product_id}`, sku: item.product?.sku || item.sku || '', image: getImage(item.product || item), quantity: Number(item.quantity || 1), unitPrice: Number(item.price ?? item.unitPrice ?? 0), discount: Number(item.discount || 0), stock: Number(item.stock ?? item.availableStock ?? 999999), sizeId: item.sizeId ?? item.size_id ?? item.size?.id ?? item.productSizeId ?? null, colorId: item.colorId ?? item.color_id ?? item.color?.id ?? item.productColorId ?? null })))
  ...
  useEffect(() => {
    Promise.all([getWarehouses(), getCustomers(), getProducts(), getCategories(), getBrands()]).then(([warehouses, customers, products, categories, brands]) => {
      setData((current) => ({ ...current, warehouses, customers, products, categories, brands }))
  ...
  useEffect(() => {
    ...
    setCart((current) => current.map((item) => {
      ...
      return { ...item, name: item.name?.startsWith('Product #') ? product.name : item.name, sku: item.sku || product.sku || '', image: item.image || getImage(product), stock: item.stock ?? availableStock(product), sizeId: item.sizeId ?? firstOptionId(product, ['sizes', 'sizeIds', 'sizeId']), colorId: item.colorId ?? firstOptionId(product, ['colors', 'colorIds', 'colorId']) }
    }))
  }, [data.products])
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      // Resuming a draft must update that draft, not create a second entry.
      await (resumedDraftId ? updatePosDraft(resumedDraftId, draftPayload) : createPosDraft(draftPayload))
  ...
  const completeSale = async () => {
    ...
    try {
      await createPosSale({ warehouseId: Number(form.warehouseId), customerId: Number(form.customerId), discount: Number(form.discount), paidAmount, paymentMethod: form.paymentMethod, items: cart.map(({ productId, sizeId, colorId, quantity, unitPrice, discount }) => ({ productId: Number(productId), sizeId: sizeId ? Number(sizeId) : null, colorId: colorId ? Number(colorId) : null, quantity: Number(quantity), price: Number(unitPrice), discount: Number(discount) })) })
    } catch (requestError) {
      const details = requestError.response?.data?.errors
      const detailMessage = Array.isArray(details) ? details.map((item) => item.message ?? item.msg).filter(Boolean).join(' ') : ''
      const apiMessage = detailMessage || requestError.response?.data?.message || ''
      const needsLogin = requestError.response?.status === 401 || /missing bearer|access token|unauthorized/i.test(apiMessage)
      setError(needsLogin ? 'Your login session has expired. Please log out, sign in again, and then complete the sale.' : apiMessage || 'The POS sale could not be created. The server returned an internal error.')
      setSaving(false)
      return
    }

    // The sale is already complete at this point. A draft clean-up error must
    // never make the user retry the sale and accidentally create it twice.
    if (resumedDraftId) {
      try {
        await deletePosDraft(resumedDraftId)
      } catch (requestError) {
        console.warn('POS sale completed, but its draft could not be deleted.', requestError)
      }
    }
    try {
      sessionStorage.removeItem(posDraftKey)
  ...
  return <AdminLayout title="Point of Sale"><div className="pos-page">
    ...
    <div className="pos-workspace">
      ...
      <aside className="pos-cart"><div className="pos-cart-header"><div><ShoppingCart size={20} /><h2>Current Sale</h2></div><span>{cart.reduce((total, item) => total + Number(item.quantity), 0)} items</span></div>
        ...
        <div className="pos-payment"><div className="pos-total-row"><span>Subtotal</span><strong>৳{money(subtotal)}</strong></div><label>Order Discount<input type="number" min="0" max={subtotal} value={form.discount} onChange={(event) => setForm((current) => ({ ...current, discount: event.target.value }))} /></label><div className="pos-total-row grand"><span>Grand Total</span><strong>৳{money(grandTotal)}</strong></div><div className="pos-payment-grid"><label>Payment Method<select value={form.paymentMethod} onChange={(event) => setForm((current) => ({ ...current, paymentMethod: event.target.value }))}><option value="cash">Cash</option><option value="transfer">Bank Transfer</option><option value="cheque">Cheque</option></select></label><label>Paid Amount<input type="number" value={grandTotal} readOnly title="POS sales must be paid in full" /></label></div><label>Note<textarea value={form.note} onChange={(event) => setForm((current) => ({ ...current, note: event.target.value }))} placeholder="Counter sale" /></label><button className="pos-checkout" type="button" disabled={saving || cart.length === 0} onClick={completeSale}>{saving ? 'Completing Sale…' : `Complete Sale · ৳${money(grandTotal)}`}</button></div>
      </aside>
    </div>
  </div></AdminLayout>
}
