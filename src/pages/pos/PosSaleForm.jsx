...
export default function PosSaleForm() {
  ...
  const [form, setForm] = useState({ warehouseId: '', customerId: '', paymentMethod: 'cash', note: '', discount: 0, paidAmount: 0 })
  const [cart, setCart] = useState([])
  const [filters, setFilters] = useState({ search: '', categoryId: '', brandId: '' })
  ...
  return <AdminLayout title="Point of Sale"><div className="pos-page">
    ...
    <div className="pos-workspace">
      ...
      <aside className="pos-cart"><div className="pos-cart-header"><div><ShoppingCart size={20} /><h2>Current Sale</h2></div><span>{cart.reduce((total, item) => total + Number(item.quantity), 0)} items</span></div>
        <div className="pos-customer"><label>Customer *<select value={form.customerId} onChange={(event) => setForm((current) => ({ ...current, customerId: event.target.value }))}><option value="">Select customer</option>{data.customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name} {customer.phone ? `— ${customer.phone}` : ''}</option>)}</select></label><Link to="/customers/create" title="Add customer"><UserPlus size={18} /></Link></div>
