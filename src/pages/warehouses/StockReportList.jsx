...
export default function StockReportList() {
  ...
  useEffect(() => {
    setLoading(true)
    getStockReports({ search }).then((data) => { setReports(data); setError('') }).catch((requestError) => setError(requestError.response?.data?.message || 'Unable to load stock reports.')).finally(() => setLoading(false))
  }, [search])
