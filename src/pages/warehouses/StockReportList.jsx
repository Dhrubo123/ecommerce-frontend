...
export default function StockReportList() {
  ...
  const [filters, setFilters] = useState({ warehouseId: '', productId: '', supplierId: '' })
  const [options, setOptions] = useState({ warehouses: [], products: [], suppliers: [], purchases: [] })

  useEffect(() => {
    setLoading(true)
    Promise.all([getStockReports(), getWarehouses(), getProducts(), getSuppliers(), getPurchases()]).then(([data, warehouses, products, suppliers, purchases]) => { setReports(data); setOptions({ warehouses, products, suppliers, purchases }); setError('') }).catch((requestError) => setError(requestError.response?.data?.message || 'Unable to load stock reports.')).finally(() => setLoading(false))
  }, [])

  const supplierProductIds = useMemo(() => options.purchases.reduce((lookup, purchase) => {
    const supplierId = purchase.supplierId ?? purchase.supplier?.id
    if (!supplierId) return lookup
    const products = purchase.items ?? purchase.purchaseItems ?? []
    products.forEach((row) => {
      const productId = row.productId ?? row.product?.id
      if (!productId) return
      const key = String(supplierId)
      if (!lookup[key]) lookup[key] = new Set()
      lookup[key].add(String(productId))
    })
    return lookup
  }, {}), [options.purchases])
  const visibleReports = reports.filter((item) => {
    ...
    const reportSupplierId = item.supplierId ?? item.supplier?.id
    const productId = item.productId ?? item.product?.id
    const matchesSupplier = !filters.supplierId || String(reportSupplierId ?? '') === filters.supplierId || supplierProductIds[filters.supplierId]?.has(String(productId))
    return matchesSearch && (!filters.warehouseId || String(item.warehouseId ?? item.warehouse?.id) === filters.warehouseId) && (!filters.productId || String(productId) === filters.productId) && matchesSupplier
  })
