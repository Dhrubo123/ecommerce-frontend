...
export default function CreditVoucherForm() {
  ...
  const selectAccount = (name, label) => <label>{label} *<select name={name} value={form[name]} onChange={change}><option value="">Select account head</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name} {account.code ? `(${account.code})` : ''}</option>)}</select></label>
