...
export default function AccountsModule({ module, create = false }) {
  ...
  const renderCell = (value, key) => key === 'isActive' || key === 'active'
    ...
    : display(value)
  return <AdminLayout title={config.title}><div className={`brand-page ${isBankReconciliation ? 'bank-reconciliation-page' : ''}`}><div className="brand-heading"><div><p>ACCOUNTS</p><h2>{create ? 'Add Payment Method' : config.title}</h2></div><button type="button" onClick={() => setRevision((v) => v + 1)}>Refresh</button></div>
