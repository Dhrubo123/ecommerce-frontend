...
export default function ProductForm() {
  ...
  useEffect(() => {
    ...
    else { setForm(blank); setSlugEdited(false); setError('') }
  }, [id, edit])
...
function Multi({ label, items, values, toggle }) {
  ...
  if (label === 'Subcategories') return <Field label="Select Subcategories"><select className="size-choice" multiple value={values.map(String)} onChange={update} disabled={!items.length} size={Math.min(Math.max(items.length, 2), 5)}>{items.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><small>{items.length ? 'Hold Ctrl / Cmd to select multiple subcategories.' : 'Select a category first.'}</small></Field>
