...
export const getPosSale=async id=>{const r=await api.get(`/admin/pos-sales/${id}`);return r.data?.data??r.data}
...
export const deletePosDraft=async id=>{const r=await api.delete(`/admin/pos-sales/drafts/${id}`);if(r.data?.success===false)throw new Error(r.data.message||'Unable to delete POS draft.');return true}
