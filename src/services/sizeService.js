...
export async function createSize(size) { return toUi(unwrap(await api.post('/admin/sizes', { name: size.name, isActive: size.isActive }))) }
export async function updateSize(id, size) { return toUi(unwrap(await api.patch(`/admin/sizes/${id}`, { name: size.name, isActive: size.isActive }))) }
