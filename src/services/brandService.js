...
const toUpdatePayload = (brand) => ({
  name: String(brand.name ?? '').trim(),
  slug: String(brand.slug ?? '').trim(),
  isActive: asBoolean(brand.isActive, brand.status === 'active'),
})
...
// The current PATCH endpoint accepts JSON. Image replacement requires a backend
// multipart PATCH handler or a dedicated endpoint such as POST /admin/brands/:id/image.
export async function updateBrand(id, brand) { return toUi(unwrap(await api.patch(`/admin/brands/${id}`, toUpdatePayload(brand), { headers: { 'Content-Type': 'application/json' } }))) }
