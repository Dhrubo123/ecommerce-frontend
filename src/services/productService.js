...
const toFormData = (product) => { const form = new FormData(); ['name','slug','shortDescription','description','categoryId','brandId','unitId','sku','weightKg','buyingPrice','sellingPrice','discountType','discount','metaTitle','metaDescription','metaKeywords'].forEach((key) => append(form, key, product[key])); append(form, 'subcategoryIds', JSON.stringify(idsFrom(product.subcategoryIds))); append(form, 'colorIds', JSON.stringify(idsFrom(product.colorIds))); const sizes = (product.sizes ?? []).map((size) => typeof size === 'object' ? size : { sizeId: Number(size) }); append(form, 'sizes', JSON.stringify(sizes)); form.append('isActive', String(product.isActive ?? product.status === 'active')); Array.from(product.thumbnailImages ?? []).filter((file) => file instanceof File).forEach((file) => form.append('thumbnailImages', file)); Array.from(product.additionalImages ?? []).filter((file) => file instanceof File).forEach((file) => form.append('additionalImages', file)); return form }
