...
const toFormData = (product) => { const form = new FormData(); ['name','slug','shortDescription','description','categoryId','subcategoryId','brandId','unitId','sku','weightKg','buyingPrice','sellingPrice','discountType','discount','stockQuantity','metaTitle','metaDescription','metaKeywords'].forEach((key) => append(form, key, product[key])); append(form, 'colorIds', JSON.stringify(product.colorIds ?? [])); append(form, 'sizeIds', JSON.stringify(product.sizeIds ?? [])); form.append('isActive', String(product.isActive ?? product.status === 'active')); Array.from(product.thumbnailImages ?? []).forEach((file) => form.append('thumbnailImages', file)); Array.from(product.additionalImages ?? []).forEach((file) => form.append('additionalImages', file)); return form }
