...
// The deployed purchase API updates through PATCH /admin/purchases with the
// same purchase payload used by the backend's documented Postman request.
export const updatePurchase = async (_id, data) => { const response = await api.patch('/admin/purchases', data); return response.data?.data ?? response.data }
