...
export async function createUnit(unit) { return toUi(unwrap(await api.post('/admin/units', { name: unit.name, isActive: asBoolean(unit.isActive) }))) }
export async function updateUnit(id, unit) { return toUi(unwrap(await api.patch(`/admin/units/${id}`, { name: unit.name, isActive: asBoolean(unit.isActive) }))) }
